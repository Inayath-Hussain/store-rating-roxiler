...
import { tryCatchWrapper } from "../../controllers/tryCatchWrapper";
...
const middleware: RequestHandler = async (req, res, next) => {
    ...
    // get user id from db
    const _getUserId = async (userId: number) => {
    ...
    if (!refreshToken) {
        ...
        const user_id = await _getUserId(result.payload.userId);
    ...
    if (!accessToken) {
        ...
        // create access token
        const newAccessToken = await createAccessToken({ userId: result.userId })


        const user_id = await _getUserId(result.userId)
        if (user_id === null) return invalidResponse();
